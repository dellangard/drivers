...
void button_state_changed_callback(BaseType_t *pxHigherPriorityTaskWoken, gpio_button_driver_descriptor_t *button_dd);
...
//            //Send event to queue
//            button_event = button_short_press;
//            xQueueSendFromISR(button_dd->xButtonEventQueue, &button_event, pxHigherPriorityTaskWoken);

            xTimerStopFromISR(button_dd->xButtonTimer, pxHigherPriorityTaskWoken);

            //Start 'front' deglitching timer
            xTimerChangePeriodFromISR(button_dd->xButtonTimer, button_dd->deglitching_time_ticks, pxHigherPriorityTaskWoken);
...
void button_register_driver(gpio_button_driver_descriptor_t *button_dd)
{
    ...
    if(exti_manager_register_handler(button_dd, &button_state_changed_callback, button_dd->gpio) == driver_register_ok)
    {
        //Get NVIC vector number for given EXTI channel
        uint8_t nvic_vector = exti_manager_get_exti_nvic_vector( button_dd->gpio );
    ...
    // Create a queue for holding maximum of 'event_queue_length' events.
    button_dd->xButtonEventQueue = xQueueCreate( button_dd->event_queue_length, sizeof( button_events_t* ) );
